define(["jquery",
  ...
  //, parentRef, contactData
  prepareFileUploads : function (fileList,parentId,observer,response){
  ...
  checkForUploads : function (){
    ...
    if(this.currentIndex < this.files.length){
      ...
      this.currentFile = this.files[this.currentIndex];//0];
      ...
      this.observer.success(this.response);
    }
  },
    ...
    if(this.byteChunkArray.length > 0){
      ...
      reader.onloadend = function(evt){
        if(evt.target.readyState == FileReader.DONE){ //Make sure read was successful, DONE == 2
          ...
          umProxy.postBlobContent(that.parentId,attachmentId,that.currentFile.name,that.currentFile.type,base64value,function(result,event){
